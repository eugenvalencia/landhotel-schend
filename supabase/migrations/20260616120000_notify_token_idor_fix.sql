-- P1-Sicherheitsfix: IDOR in der öffentlichen Edge Function `notify-schend` schließen.
--
-- Problem (Security-Review 2026-06-16): notify-schend ist öffentlich (--no-verify-jwt,
-- CORS) und lädt zu einer beliebigen booking_id per Service-Role Gast-PII + löst Mails
-- aus — ohne Nachweis, dass der Aufrufer zu dieser Buchung gehört. Jeder, der eine
-- booking_id kennt/rät, konnte fremde Gast-Mails (inkl. verbindlicher "Bestätigung")
-- triggern und das Hotel-Postfach fluten.
--
-- Fix: pro Buchung ein hochentropes, unrat­bares Geheimnis `notify_token`.
--   - create_booking generiert es server-seitig und GIBT es an den (anonymen) Gast
--     zurück → nur dieser eine Gast kennt es.
--   - Eingeloggte Hotel-User lesen es per RLS aus der eigenen Buchung (für die
--     verbindliche Bestätigungs-Mail aus dem Dashboard).
--   - notify-schend sendet NUR, wenn booking_id UND token zusammenpassen (sonst 403).
--
-- Rein additiv. Der Spalten-Default deckt JEDEN Insert-Pfad ab (create_booking,
-- create_internal_booking, künftige) und backfillt bestehende Zeilen automatisch —
-- keine weitere RPC muss angefasst werden.

-- ---------- 1. notify_token-Spalte (Default backfillt alle Zeilen) ----------
-- 244-Bit-Geheimnis aus zwei gen_random_uuid() (built-in, keine pgcrypto-Abhängigkeit).
ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS notify_token TEXT NOT NULL
  DEFAULT (replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', ''));

-- Der Token ist ein Geheimnis: niemals breit lesbar machen. Die bestehende RLS auf
-- bookings (nur Tenant-Admins SELECT) deckt das ab; anon kann bookings nicht lesen,
-- bekommt den Token ausschließlich über den create_booking-Rückgabewert.

-- ---------- 2. create_booking() um notify_token erweitern ----------
-- Identische Signatur + Logik wie 20260604190000; einzige Änderungen:
--   + Deklaration v_notify_token, + RETURNING in den Insert, + Feld im JSON-Return.
...
BEGIN
  ...
  RETURNING notify_token INTO v_notify_token;
